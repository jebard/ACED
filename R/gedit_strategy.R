...
execute_gedit <- function(res){
  ...
  message(paste0(py_config()$python," ",package_info(include_base = F)$library[1],"/GEDIT3.py -mix $PWD/MixtureQuery.csv -ref $PWD/RefObj.",res,".csv -outFile $PWD/GEDIT_Deconv"))
  system(paste0(py_config()$python," ",package_info(include_base = F)$library[1],"/GEDIT3.py -mix $PWD/MixtureQuery.csv -ref $PWD/RefObj.",res,".csv -outFile $PWD/GEDIT_Deconv"),TRUE)
}
