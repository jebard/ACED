...
gedit_prep_reference <- function(ref_obj,res){
  ...
  write.csv(file=paste0("gedit3_outputs/RefObj.",res,".csv"),Seurat::AverageExpression(ref_obj,assays = "RNA",slot = "counts")$RNA)
}

gedit_prep_query <- function(query_obj){
  ### save out the mixture sets for each original sample
  Seurat::Idents(query_obj) <- "orig.ident"
  write.csv(file="gedit3_outputs/MixtureQuery.csv",Seurat::AverageExpression(query_obj,assays = "RNA",slot = "counts")$RNA)
}
